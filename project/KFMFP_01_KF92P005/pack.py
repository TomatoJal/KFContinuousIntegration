...
def pack(config):
    ...
    for key in config['IAR']['firmware']:
        ...
        ver2_FF[1] = 0xFF
        tmp_path = f'./temp/Camel_03_{key}_V{ver1.hex()}-V{ver2.hex()}'
        ...
        #                        Camel_03_KF13A017_100A-DC_APP1_V 1016
        tag = key[:key.find('-')]
        app1.save(f"{tmp_path}/Camel_03_KF13A017_{tag}_APP1_V{ver1.hex()}_KFM_{tag}_DLMS.bin")
        ...
        app1.save(f"{tmp_path}/Camel_03_KF13A017_{tag}_APP1_V{ver1_FF.hex().upper()}_KFM_{tag}_DLMS.bin")
        # 处理APP2
        # 填充0xFF
        ...
        app2.save(f"{tmp_path}/Camel_03_KF13A017_{tag}_APP2_V{ver2.hex()}_KFM_{tag}_DLMS.bin")
        # 还原文件
        app2.reset()
        # 修改版本号为0xFF
        ...
        app2.save(f"{tmp_path}/Camel_03_KF13A017_{tag}_APP2_V{ver2_FF.hex().upper()}_KFM_{tag}_DLMS.bin")
        # 烧录文件
        app1.reset()
        app2.reset()
        combine = boot + app1 + app2
        combine.save(f"{tmp_path}/Camel_03_KF13A017_{tag}_V{ver1.hex()}-V{ver2.hex()}.bin")
        combine.bin2hex(f"{tmp_path}/Camel_03_KF13A017_{tag}_V{ver1.hex()}-V{ver2.hex()}.hex")

    shutil.make_archive(f'./project/camel/output/Camel_03_KF13A017_'
                        ...
                        f'{time.strftime("%Y_%m_%d_%H_%M", time.localtime())}', 'zip', './temp')
